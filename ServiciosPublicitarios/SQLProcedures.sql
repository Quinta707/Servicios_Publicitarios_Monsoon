...
--**************LISTADO DE INSUMOS POR SERVICIO ***********
--**************  VISTA ******************--
GO
CREATE OR ALTER VIEW pbli.VW_tbInsumosPorServicio
AS
SELECT	T1.insu_Id, 
		insu_Nombre, 
		cate_Id, 
		...
		inse_UsuCreacion, 
		inse_FechaCreacion, 
		inse_UsuModificacion, 
		inse_FechaModificacion, 
		inse_Estado
FROM	pbli.tbInsumos T1 INNER JOIN pbli.tbInsumosPorServicios T2
ON		T1.insu_Id = T2.insu_Id
WHERE	insu_Estado = 1

--**************  INDEX ******************--
GO
CREATE OR ALTER PROCEDURE pbli.UDP_tbInsumosPorServicio_Lista
(
@serv_Id INT
)
AS
...
--**************  CREATE ******************--
GO
CREATE OR ALTER PROCEDURE pbli.UDP_tbInsumosPorServicio_Insert
(
@serv_Id			INT, 
@insu_Id			INT, 
@inse_UsuCreacion	INT
)
AS
BEGIN
	BEGIN TRY
		INSERT INTO pbli.tbInsumosPorServicios([serv_Id], [insu_Id], [inse_UsuCreacion])
		VALUES	(@serv_Id, @insu_Id, @inse_UsuCreacion)
		SELECT 1 codeStatus
	END TRY
	BEGIN CATCH
		SELECT 0 codeStatus
...
--************** DELETE *****************--
GO
CREATE OR ALTER PROCEDURE pbli.UDP_tbInsumosPorServicio_Delete
(@inse_Id INT)
AS
BEGIN
	BEGIN TRY
		UPDATE [pbli].[tbInsumosPorServicios]
		SET inse_Estado = 0
		WHERE inse_Id = @inse_Id
...
--************** DDL DE INSUMOS POR CATEGORIA *****************--
GO
CREATE OR ALTER PROCEDURE pbli.UDP_tbInsumos_Cate_DDL 
(@cate_Id INT)
AS
BEGIN
	SELECT * FROM pbli.VW_tbInsumos
