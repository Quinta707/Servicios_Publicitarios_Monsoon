...
namespace ServiciosPublicitarios.BusinessLogic.Service
{
    public class PublicidadService
    {
        ...
        public ServiceResult InsumosDDL(int id)
        {
            var result = new ServiceResult();
            try
            {
                var list = _insumoRepository.InsumosDDL(id);
                return result.Ok(list);
            }
            catch (Exception)
            {
        ...
        public ServiceResult EliminarInsumosPorServicio(tbInsumosPorServicios item)
        {
            var result = new ServiceResult();
            try
            {
                var map = _insumosPorServicioRepository.Delete(item);
                if (map.CodeStatus > 0)
                {
                    return result.Ok(map);
                }
        ...
        public ServiceResult InsertarInsumosPorServicio(tbInsumosPorServicios item)
        {
            var result = new ServiceResult();
            try
            {
                var map = _insumosPorServicioRepository.Insert(item);
                if (map.CodeStatus > 0)
                {
                    return result.Ok(map);
                }
